...
```javascript
{
    ...
    console.log(ulEl, btn);
    
    
    btn.onclick = () => {
      for(let idx in menus) { //idx =0 idx<menus idx++
        const liEl = document.createElement('li')
        liEl.innerText = menus[idx];
        ulEl.appendChild(liEl)

      }
      for(let item of menus) { //idx =0 idx<menus idx++
        const liEl = document.createElement('li')
        liEl.innerText = item;
        ulEl.appendChild(liEl)

      }
      menus.forEach((item, idx)=>  { //idx =0 idx<menus idx++
        const liEl = document.createElement('li')
        liEl.innerText = item +'/' + idx;
        ulEl.appendChild(liEl);

      })
      // ulEl.innerHTML = `<li>${menus[0]}</li>`

...
// 2 . name 속성을 이용한 접근 방식 (암기)
// 태그에 name 속성으로 정의된 이름으로 접근하는 방식 
{
  const formOrder = document.order;
  // console.log(formOrder.children); 폼요소의 자식요소
...
// </div>
  ...
  function displaySelect() {
    ...
    console.log(selected + "를 선택하셨습니다"); //전기전자공학과를 선택하셨습니다
      console.log(selectedValue); //computer
  }
...
```javascript
{
  // const save = {}
  const save = []
  console.log(document.testForm.subject.value); //subject(name) 식별하기 위한 의미 배열로 들어간다
  const radioCheck = document.testForm.subject;
  radioCheck.forEach((item) => {
    item.addEventListener("change", (e) => {
      // console.log(e.target.value); //writing
      // 선택 되어지 데이터를 객체를 저장
      save[e.target.name] = e.target.value 
      console.log(save); //{grammar: 'grammar', writing: 'writing', speaking: 'speaking'} 클릭시 포함 되며 들어간다

      // 배열로 저장
      save.push({[e.target.name] : e.target.value})
      console.log(save);
      
    });
  });

  boxs = document.querySelectorAll("[name=mailing]");
  console.log(checkboxs); //[input#news, input#dialog, input#pops]

  console.log(document.querySelector("[name=mailing]:checked")); //     <label><input type="checkbox" id="dialog" name="mailing" value="dialog" / checked>5분 회화</label> 체크 쓴 값이 나온다

  checkboxs.forEach((item) => {
    console.log(item);
    item.addEventListener("change", (e) => {
      console.log(e.target.value); // 클릭 시 해당하는 뜸 ex)dialog
    });
  });

  const all = document.querySelectorAll(".selectAll");

//체크박스 버튼 클릭하면 전체 선택 & 해제

  all[0].addEventListener("click", (e) => {
    e.preventDefault();
    for (idx in checkboxs) {
      checkboxs[idx].checked = true
    }
  });
  all[1].addEventListener("click", (e) => {
    e.preventDefault();
    for (idx in checkboxs) {
      checkboxs[idx].checked = false
    }
  });
